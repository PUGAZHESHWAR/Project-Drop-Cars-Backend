...
def get_all_admin_orders(db: Session, skip: int = 0, limit: int = 100) -> tuple[List[AdminOrderDetailResponse], int]:
    ...
    for order in orders:
        ...
        order_response = AdminOrderDetailResponse(
            ...
            customer_name=order.customer_name,
            customer_number=order.customer_number,
            trip_status=order.trip_status if order.cancelled_by is None else "AUTOCANCELLED",
            pick_near_city=order.pick_near_city,
            trip_distance=order.trip_distance,
            trip_time=order.trip_time,
            estimated_price=order.estimated_price,
...
def get_vehicle_owner_orders_by_assignment_status(
    ...
    for order, assignment in query.all():
        ...
        if order.source == "NEW_ORDERS":
            ...
            if hourly_rental:
                pickup_notes = hourly_rental.pickup_notes
                charges_to_deduct = int(order.vendor_price - order.estimated_price) if hourly_rental else 0
